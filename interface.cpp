...
void biliard_geometry(bl::Biliard &bil) {
  ...
  double l, y1, y2;
  std::cout << "Inserisci la lunghezza del biliardo: ( l < 1200) \n";
  std::cin >> l;
  std::cout
      << "Inserisci la semiampiezza del lato sinistro del biliardo: ( y < 300) \n";
  std::cin >> y1;
  std::cout
      << "Inserisci la semiampiezza del lato destro del biliardo: (y < 300)\n";
  std::cin >> y2;
...
void Biliard::Dynamic_Trace(Ball &b, std::vector<Point> &coll) {
  if (std::abs(b.y_coord) > upper_cushion_.y_intercept ||
    ...
    while (collision_point.x <= lenght_ && collision_point.x >= 0) {
      // incl.push_back(ball_path.slope);
...
//  const std::function<void(sf::RenderWindow&)> &draw_bg)
{
  ...
  while (window.isOpen() && traveled < total_dist) {
    ...
    float step = speed * dt;
    if (traveled + step > total_dist) step = total_dist - traveled;
...
void trajectory(const std::vector<Point> &coll, bl::Biliard &bil,
                float speed = 100.f) {
  sf::RenderWindow window(sf::VideoMode(1400.f, 900.f), "Biliardo triangolare");
  window.setPosition(sf::Vector2i{10, 10});

  // Crea una view con origine a sinistra e centro in mezzo
  sf::View view;
  view.setSize(1400.f, -900.f);
  ...
  // auto draw_bg = [&](sf::RenderWindow &win) {
  //   win.draw(upper_rect);
  //   win.draw(lower_rect);
  // };
...
// void Biliard::Graphic_Dynamic(Ball &b) {
//   sf::RenderWindow window(sf::VideoMode(1400.f, 900.f), "Biliardo
//   triangolare"); window.setPosition(sf::Vector2i{10, 10});
...
//       static_cast<float>(std::sqrt(this->left_limit() * this->left_limit()
//       +
//                                    this->lenght() * this->lenght()));
//   upper_rect.setSize(sf::Vector2f{upper_rect_lenght, 5.f});
//   upper_rect.setPosition(100.f, static_cast<float>(this->left_limit()));
//   float upper_rect_incl =
//       static_cast<float>((180 * std::atan(this->incl())) / M_PI);
//   upper_rect.setRotation(upper_rect_incl);
//   sf::RectangleShape lower_rect;
//   lower_rect.setFillColor(sf::Color::White);
//   lower_rect.setSize(sf::Vector2f{upper_rect_lenght, 5.f});
//   float lower_rect_pos = static_cast<float>(-this->left_limit());
//   lower_rect.setPosition(100.f, lower_rect_pos);
//   float lower_rect_incl = static_cast<float>(360 - upper_rect_incl);
//   lower_rect.setRotation(lower_rect_incl);
...
//   ball.setPosition(sf::Vector2f{100.f, 0.f});
